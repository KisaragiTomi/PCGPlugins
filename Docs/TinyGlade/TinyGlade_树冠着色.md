...
| # | 机制 | 一句话 | 本项目 |
| --- | --- | --- | --- |
| 1 | **深度浮雕** | 平面卡片在深度缓冲里变成 1.6 m 厚的叶球，两树相交得到弯交线 | 未做，见「移植缺口」①  |
| 2 | **逆光旁路** | 相机与太阳异侧时，`N·L` 与厚度遮蔽两项被同时旁路，只剩边缘项 | 未做，见「移植缺口」② |
| 3 | **随机 UV 采色板** | 用一张 512² 色板当随机取色器，零叶片贴图 | 未做，与 `M_TG_Grass` 的零贴图路子同源 |
| 4 | **缩放淡出** | 快被 cone-cull 掉的叶簇先缩成一点再消失，无 pop | 未做，与草的 `packed0>>8` 淡出同思路 |
| 5 | **height 一通道三用** | 同一个 8 位通道跑深度浮雕、花的高度合成、间接光遮蔽 | 未做 |
...
**① 深度浮雕**（对应速查表 1）。UE 的 `Pixel Depth Offset` 只能把像素往**远**推，且吃掉 early-Z；
TG 是"VS 拉近 + PS 推回"。等价映射是：卡片顶点整体沿视线抬 1.6 m（WPO），再用 PDO 按 `1 - height`
推回。直接照抄 TG 的符号会得到反向的凸起。

**② 逆光旁路**（对应速查表 2）。`MSM_TWO_SIDED_FOLIAGE` 给的是透射项，但没有"逆光时同时旁路 `N·L`
与 AO"这条。要在自定义光照或后处理里补，否则逆光边缘出不来。

**③ 随机 UV 采色板**。零成本，与 `M_TG_Grass` 的零贴图路子同源，可直接照抄 mode 1 的两次采样式子。

**④ 缩放淡出**。`(1 - fade²)` 的无 pop 剔除，与草上已落地的 `packed0 >> 8` 缩宽淡出是同一思路的另一形态。
