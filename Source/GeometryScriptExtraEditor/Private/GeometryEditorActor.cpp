	...
	TArray<FVineLinePointScaleData>& OutLinePointScales)
{
	...
	for (int32 LineIdx = 0; LineIdx < Lines.Num(); ++LineIdx)
	{
		...
		if (Line.Path.IsValid() && Line.Path->Num() >= 3)
		{
			...
			FVineLinePointScaleData& PreparedScales = OutLinePointScales.AddDefaulted_GetRef();
			BuildPreparedLinePointScales(Line, InputPointScales, Line.Path->Num(), FallbackScale, PreparedScales.Values);
		}
	}
	...
	bool bRecomputeNormals)
{
	...
	// Decode packed UV from Vertex.W for each vertex
	// Shader packs: upper 16 bits = f16(U), lower 16 bits = f16(V)
	TArray<FVector2f> DecodedUVs;
	DecodedUVs.SetNum(Vertices.Num());
	for (int32 i = 0; i < Vertices.Num(); ++i)
	{
		const uint32 PackedUV = *reinterpret_cast<const uint32*>(&Vertices[i].W);
		const uint16 UHalf = static_cast<uint16>(PackedUV >> 16u);
		const uint16 VHalf = static_cast<uint16>(PackedUV & 0xFFFFu);
		DecodedUVs[i] = FVector2f(
			FPlatformMath::LoadHalf(&UHalf),
			FPlatformMath::LoadHalf(&VHalf));
	}
	...
	for (int32 i = 0; i < Vertices.Num(); ++i)
	{
		UVElementIDs[i] = UVOverlay->AppendElement(DecodedUVs[i]);
	}
...
static bool ResolveVineReferenceData(
	...
	UInstancedStaticMeshComponent*& OutDisplayComponent)
{
	...
	{
		OutTransformArray = &Container->TubeVineSourceTransforms;
	...
	{
		OutTransformArray = &Container->PlaneVineSourceTransforms;
	...
	{
		OutTransformArray = &Container->GrowTargetTransforms;
...
{
	RebuildVineDisplayInstances(GrowTarget, GrowTargetTransforms);
	RebuildVineDisplayInstances(TubeVineSource, TubeVineSourceTransforms);
	RebuildVineDisplayInstances(PlaneVineSource, PlaneVineSourceTransforms);
}
...
void AVineContainer::ImportFoliageToTransformArray(UFoliageType* InFoliageType)
{
	...
	TArray<FTransform>* TransformArray = nullptr;
	UInstancedStaticMeshComponent* DisplayComponent = nullptr;
	if (!ResolveVineReferenceData(this, InFoliageType, TransformArray, DisplayComponent) || !TransformArray)
	{
		return;
	}

	Modify();
	TransformArray->Append(Transforms);
	MarkPackageDirty();
	RebuildVineDisplayInstances(DisplayComponent, *TransformArray);
...
void AVineContainer::ExportTransformArrayToFoliage(UFoliageType* InFoliageType)
{
	...
	FoliageInstances.Reserve(TransformArray->Num());

	for (const FTransform& InstanceTransform : *TransformArray)
	{
...
void AVineContainer::VisVine( bool MainVine)
{
	...
	TArray<float> TempLineSourceScales;
	TempLineSourceScales.Reserve(Lines.Num());
	TArray<FVineLinePointScaleData> TempLinePointScales;
	TempLinePointScales.Reserve(Lines.Num());
	for (int32 i = 0; i < Lines.Num(); i++)
	{
		...
		*PolyPath.Path = PathVertices;
		if (LineSourceLocations.IsValidIndex(i))
		{
			ApplyVineVisualizationSCPointOffset(PolyPath, LineSourceLocations[i]);
		}
		TempLines.Add(PolyPath);
		const float FallbackScale = LineSourceScales.IsValidIndex(i) ? LineSourceScales[i] : 1.0f;
		const TArray<float>* InputPointScales = LinePointScales.IsValidIndex(i) ? &LinePointScales[i].Values : nullptr;
		TempLineSourceScales.Add(FallbackScale);
		FVineLinePointScaleData& TempScaleData = TempLinePointScales.AddDefaulted_GetRef();
		BuildPreparedLinePointScales(PolyPath, InputPointScales, PolyPath.Path->Num(), FallbackScale, TempScaleData.Values);
	}

	auto RebuildPointScalesForEditedLine = [](
	...
	for (int32 LineIdx = 0; LineIdx < TempLines.Num(); ++LineIdx)
	{
		FGeometryScriptPolyPath& Line = TempLines[LineIdx];
		TArray<float>& CurrentPointScales = TempLinePointScales[LineIdx].Values;
		const float FallbackScale = TempLineSourceScales.IsValidIndex(LineIdx) ? TempLineSourceScales[LineIdx] : 1.0f;
		...
		Line = UPolyLine::SmoothLine(Line, 3);
		RebuildPointScalesForEditedLine(PreviousLine, Line, FallbackScale, CurrentPointScales);
...
bool AVineContainer::VisVineGPU(bool MainVine)
{
	...
	if (!PrepareVineVisualizationLinesCPU(Lines, VV, MainVine, LineSourceScales, LineSourceLocations, LinePointScales, PreparedLines, PreparedLineSourceScales, PreparedLinePointScales))
	{
...
UDynamicMesh* AVineContainer::GenerateVines(float ExtrudeScale, bool Result, bool MultThread)
{
	...
	const TArray<FTransform>& TubeSourceTransforms = TubeVineSourceTransforms;
	const TArray<FTransform>& PlaneSourceTransforms = PlaneVineSourceTransforms;
	const TArray<FTransform>& TargetTransforms = GrowTargetTransforms;
	...
	// 5. 可视化
	{
		...
		VisVine(true);   // Tube 可视化 (swept polygon)
	}
	{
		GV_ACTOR_TIME_SCOPE(TEXT("AVineContainer.GenerateVines.VisPlaneVine"));
		VisVine(false);  // Plane 可视化 (swept polyline)
	}

	if (UDynamicMeshComponent* MeshComponent = GetDynamicMeshComponent())
	{
		if (UDynamicMesh* ContainerMesh = MeshComponent->GetDynamicMesh())
		{
...
void AVineContainer::GenerateVineAction()
{
	...
	const int32 LastTargetIndex = GrowTargetTransforms.Num() - 1;
	...
	for (const FTransform& TargetTransform : GrowTargetTransforms)
	{
