...
UE_FLDImpl::UE_FLDImpl(int _length_threshold, float _distance_threshold,
        double _canny_th1, double _canny_th2, int _canny_aperture_size, bool _do_merge)
...
void UE_FLDImpl::detect(InputArray _image, TArray<Vec4f>& _lines)
{
...
void UE_FLDImpl::drawSegments(InputOutputArray _image, InputArray lines, bool draw_arrow)
{
...
void UE_FLDImpl::mergeLines(const SEGMENT& seg1, const SEGMENT& seg2, SEGMENT& seg_merged)
{
...
double UE_FLDImpl::distPointLine(const Mat& p, Mat& l)
{
...
bool UE_FLDImpl::mergeSegments(const SEGMENT& seg1, const SEGMENT& seg2, SEGMENT& seg_merged)
{
...
template<class T>
    void UE_FLDImpl::incidentPoint(const Mat& l, T& pt)
    {
...
void UE_FLDImpl::extractSegments(const std::vector<Point2i>& points, std::vector<SEGMENT>& segments )
{
    bool is_line;
...
void UE_FLDImpl::pointInboardTest(const Mat& src, Point2i& pt)
{
    pt.x = pt.x <= 5 ? 5 : pt.x >= src.cols - 5 ? src.cols - 5 : pt.x;
    pt.y = pt.y <= 5 ? 5 : pt.y >= src.rows - 5 ? src.rows - 5 : pt.y;
}

bool UE_FLDImpl::getPointChain(const Mat& img, Point pt,
...
void UE_FLDImpl::lineDetection(const Mat& src, std::vector<SEGMENT>& segments_all)
{
    int r, c;
...
inline void UE_FLDImpl::getAngle(SEGMENT& seg)
{
    seg.angle = (float)(fastAtan2(seg.y2 - seg.y1, seg.x2 - seg.x1) / 180.0f * CV_PI);
}

void UE_FLDImpl::additionalOperationsOnSegment(const Mat& src, SEGMENT& seg)
{
...
void UE_FLDImpl::drawSegment(Mat& mat, const SEGMENT& seg, Scalar bgr, int thickness, bool directed)
{
    double gap = 10.0;
    double ang = (double)seg.angle;
    double arrow_angle = 30.0;
